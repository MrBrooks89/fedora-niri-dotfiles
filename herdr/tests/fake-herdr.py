...
if args==["integration","status"]:
    print("codex: current (v10) (/tmp/fake)")
    print("opencode: current (v10) (/tmp/fake)")
    raise SystemExit
selected=False
...
if args[:2]==["agent","start"]:
    mutation(); role=args[2]; pane=next(p for p in state["panes"] if p["pane_id"]==option("--pane")); pane.update({"agent":option("--kind"),"agent_status":"idle","agent_session":{"kind":"id","value":"session-"+role}})
