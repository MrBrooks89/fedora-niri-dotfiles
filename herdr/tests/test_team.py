...
class EndToEnd(unittest.TestCase):
    def test_fresh_setup_and_noop_rerun_without_herdr_session(self):
        ...
        state=h.state(); self.assertEqual([w["label"] for w in state["workspaces"]],["Dotfiles Team"]); self.assertEqual(sorted(t["label"] for t in state["tabs"]),["Build","Review"]); self.assertEqual(sorted(a["name"] for a in state["agents"]),["coordinator","implementation","integration","release","security","validation"]); self.assertEqual({a["name"]:a["agent"] for a in state["agents"]},{"coordinator":"codex","implementation":"codex","integration":"codex","validation":"opencode","security":"opencode","release":"opencode"}); self.assertEqual(len(state["panes"]),6)
