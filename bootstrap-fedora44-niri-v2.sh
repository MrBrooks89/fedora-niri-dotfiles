...
#       --dotfiles-repo https://github.com/YOURUSER/dotfiles.git \
#       --with-nerd-font \
#       --with-satty-copr \
#       --with-docker \
#       --with-containerlab
...
usage() {
    cat <<'EOF'
Usage: bootstrap-fedora44-niri-v2.sh [options]

Options:
  --dotfiles-repo URL      Clone/update a dotfiles Git repository
  --dotfiles-branch NAME   Optional branch to use
  --with-nerd-font         Install JetBrainsMono Nerd Font in ~/.local/share/fonts
  ...
  --all                    Enable all optional software (network remains interactive)
...
echo "==> Installing base CLI/development utilities"
sudo dnf -y install \
...
# Ensure ~/.local/bin is available to future login shells.
...
if [[ "$WITH_NERD_FONT" -eq 1 ]]; then
    ...
    echo "      Use --with-nerd-font if your Starship/eza config needs Nerd Font glyphs."
fi
...
if [[ -n "$DOTFILES_REPO" ]]; then
    echo "==> Restoring dotfiles from Git"
    ...
    echo "NOTE: no --dotfiles-repo supplied; existing configs were left alone."
fi

# If a dotfiles repo did not provide niri/config.kdl, create a minimal fallback.
...
# Screenshot helper only if dotfiles did not provide one.
...
# Starship activation only if ~/.zshrc was not restored/provided.
...
EOF
    ...
    echo "      Add it to your dotfiles if you want Starship enabled."
fi
...
if [[ -n "$DOTFILES_REPO" ]]; then
    ...
    echo "  configs are symlinked into \$HOME"
fi
echo
echo "Important:"
echo "  If Docker/containerlab groups were changed, completely log out/in."
echo "  Then reboot to enter GDM:"
echo "      sudo reboot"
echo
echo "Verification:"
echo "  command -v niri noctalia kitty nvim eza starship"
